...
jQuery(document).ready(function(){
	...
	clearInterval(<?php echo esc_html( $intval_signup ); ?>);
				...
				jQuery('#bbp_topic_submit').attr("title", "<?php echo esc_html( __( 'Please complete the security check to submit your topic.', 'buddypress-recaptcha' ) ); ?>");
		<?php else : ?>
...
transform:scale(0.89);-webkit-transform:scale(0.89);transform-origin:0 0;-webkit-transform-origin:0 0;
			...
			if ( 'yes' === $is_enabled ) {
				...
				if ( '' === $wbc_recapcha_bbpress_topic_generation_v3 ) {
					...
					$wbc_recapcha_bbpress_topic_generation_v3 = 'no';
				}
...
grecaptcha.execute('<?php echo esc_html( $site_key ); ?>', { action: '<?php echo esc_html( $wbc_recapcha_bbpress_topic_action_v3 ); ?>' }).then(function (token) {

var recaptchaResponse = document.getElementById('wbc_recaptcha_bbpress_topic_token');
recaptchaResponse.value = token;
});
});

				<?php if ( 'yes' === $wbc_recapcha_bbpress_topic_generation_v3 ) : ?>
...
// submitval=jQuery("#bbp_reply_submit").val();
var recaptchaResponse = document.getElementById('wbc_recaptcha_bbpress_topic_token');
recaptchaResponse.value = token;
// jQuery('#new-post').prepend('<input type="hidden" name="bbp_reply_submit" value="' + submitval + '">');
...
<?php endif; ?>
	...
	public function wbr_bbpress_topic_verify_recaptcha( $response = false ) {
		...
		if ( 'v2' === $version ) {
			...
			$url = apply_filters( 'anr_google_verify_url', sprintf( 'https://www.%s/recaptcha/api/siteverify', wbr_bbpress_topic_recaptcha_domain() ) );
			...
			if ( false === $response ) {
				$response = isset( $_POST['wbc_recaptcha_bbpress_topic_token'] ) ? sanitize_text_field( wp_unslash( $_POST['wbc_recaptcha_bbpress_topic_token'] ) ) : '';
			}
