...
- **Distribution**: wbcomdesigns.com via the official EDD SL SDK with a **preset unlimited-activation key** (`d23187723b1423b54ef206a3588a1845`, baked into the registration + seeded/activated once per site on `admin_init` — BuddyNext pattern, owner decision 2026-08-04; replaced the earlier keyless mode). `item_id 1246648`. The SDK lives at **`libs/edd-sl-sdk/`** (uniform Wbcom layout, byte-identical `src/` to BuddyNext's copy which carries two fixes over upstream 1.0.3: a non-object API-response fatal guard in `Updaters/Plugin.php` and `plugins_url()`-based asset URLs in `Utilities/Path.php`). The require is guarded on `src/Versions.php` existing and degrades to a soft admin notice. Root `vendor/` is dev-only (gitignored, excluded from dist). **Do not touch the vendored SDK itself or the `item_id`; refresh it by copying BuddyNext's `libs/edd-sl-sdk/src`.**
...
- **No PSR-4 namespace**; no Composer-autoloaded plugin code. The EDD SL SDK is plain-vendored at `libs/edd-sl-sdk/` (not composer-managed); root `vendor/` holds dev tooling only.
...
|---|---|---|---|
| 2026-08-04 | Refactor (uniform SDK layout) | **EDD SL SDK moved to `libs/edd-sl-sdk/` (BuddyNext-uniform) and synced with BN's copy.** BN's `src/` carries two fixes over upstream 1.0.3 — a non-object API-response fatal guard (`Updaters/Plugin.php`) and `plugins_url()`-based asset URLs (`Utilities/Path.php`, fixes 404s on symlinked/LocalWP hosts) — now byte-identical here. Root `vendor/` became dev-only: untracked, gitignored, excluded wholesale from dist; composer no longer manages the SDK. PHPStan excludes `libs`. Refresh path: copy BN's `libs/edd-sl-sdk/src`. | `libs/edd-sl-sdk/` (moved), `recaptcha-for-buddypress.php`, `gruntfile.js`, `phpstan.neon`, `composer.json`, `composer.lock`, `.gitignore` |
| 2026-08-04 | Release (2.1.0 refresh) | **Re-released v2.1.0 with the post-July-17 fixes and a preactivated update channel.** The published zip predated the hCaptcha-fatal/ALTCHA-source/Italian fixes on `main`; tag moved to the current HEAD and the release asset replaced. SDK registration switched from `keyless` to a preset key (BuddyNext pattern: `license` arg + one-time `admin_init` activation against the store, options `buddypress-recaptcha_license_key` / `buddypress_recaptcha_preset_activated`). Added the partial-package guard (missing SDK `src/` → soft notice, never fatal) and fixed the Updates-tab SDK detection (old check named a 1.0.2 function + wrong namespace, always warned). `vendor/psr` + remaining dev-dep dirs excluded from both dist copy lists (the July zip had leaked `vendor/psr`). Verified: pristine Docker install (Reign active), store-confirmed activation, degrade test, 390px. | `recaptcha-for-buddypress.php`, `includes/admin/views/updates.php`, `includes/admin/class-bprc-admin.php`, `gruntfile.js`, `readme.txt` |
