# Wbcom CAPTCHA Manager — QA Release Checklist
...
- [ ] On a named release branch (`release/2.2.0` or equivalent), NOT on `main`
...
```bash
cd /Users/varundubey/Local Sites/wbcom-free/app/public/wp-content/plugins/buddypress-recaptcha
git status
git fetch origin
...
Wbcom CAPTCHA Manager keeps the version in multiple places. Every place must match.

- [ ] `recaptcha-for-buddypress.php` header `Version:` comment equals `2.2.0`
- [ ] `recaptcha-for-buddypress.php` `define( 'RFB_PLUGIN_VERSION', '2.2.0' );` matches
- [ ] `readme.txt` `Stable tag: 2.2.0` matches (if .org plugin)
- [ ] `package.json` `version` matches (if present)
- [ ] `composer.json` `version` matches (if present)
- [ ] `CHANGELOG.md` has a `## 2.2.0 — YYYY-MM-DD` entry with real release notes

Fast check:
```bash
grep -rE "Version:|define.*RFB_PLUGIN_VERSION|Stable tag" /Users/varundubey/Local Sites/wbcom-free/app/public/wp-content/plugins/buddypress-recaptcha \
  | grep -v vendor | grep -v node_modules
```

Every printed line should show `2.2.0`.

## 2 — Static analysis

Run from `/Users/varundubey/Local Sites/wbcom-free/app/public/wp-content/plugins/buddypress-recaptcha`.
...
- [ ] `composer phpstan` — level 5 clean, or only entries in the baseline
...
```bash
find /Users/varundubey/Local Sites/wbcom-free/app/public/wp-content/plugins/buddypress-recaptcha -name "*.php" \
...
```bash
grep -rE "\.only\(|\.skip\(" /Users/varundubey/Local Sites/wbcom-free/app/public/wp-content/plugins/buddypress-recaptcha/tests/ /Users/varundubey/Local Sites/wbcom-free/app/public/wp-content/plugins/buddypress-recaptcha/assets/ 2>/dev/null
```
...
- [ ] Text domain consistent across all files (`buddypress-recaptcha`)
- [ ] `_n()` used for pluralizable strings (not runtime `if ($count === 1)`)

```bash
# Em-dash check
grep -rE "(__|_e|_x|_n|esc_html__|esc_attr__|esc_html_e|esc_attr_e)\\([^)]*—" \
  /Users/varundubey/Local Sites/wbcom-free/app/public/wp-content/plugins/buddypress-recaptcha | grep -v vendor
```

Regenerate pot:
```bash
wp i18n make-pot /Users/varundubey/Local Sites/wbcom-free/app/public/wp-content/plugins/buddypress-recaptcha /Users/varundubey/Local Sites/wbcom-free/app/public/wp-content/plugins/buddypress-recaptcha/languages/buddypress-recaptcha.pot
```
...
- [ ] `Stable tag` matches `2.2.0`
- [ ] Changelog entry written for `2.2.0`
- [ ] Upgrade notice written for `2.2.0` if behavior changes
...
- [ ] `/Users/varundubey/Local Sites/wbcom-free/app/public/wp-content/plugins/buddypress-recaptcha/docs/qa/.last-smoke-pass.json` exists
- [ ] Report `release_version` equals `2.2.0`
...
If the report is missing or stale, run the `buddypress-recaptcha-smoke` skill.
...
- [ ] Resulting zip HAS: `recaptcha-for-buddypress.php`, `readme.txt`, `languages/*.pot`, `includes/`, `assets/`, `templates/`, `vendor/` (if runtime deps)
- [ ] Zip extracts to a folder named exactly `buddypress-recaptcha/` (not `buddypress-recaptcha-2.2.0/` or similar)
- [ ] Zip size reasonable (flag if >2× previous release)

```bash
cd /Users/varundubey/Local Sites/wbcom-free/app/public/wp-content/plugins/buddypress-recaptcha
bin/build-release.sh --output /tmp
unzip -l /tmp/buddypress-recaptcha-2.2.0.zip | head -50
ls -lh /tmp/buddypress-recaptcha-2.2.0.zip
```
...
- [ ] Install the generated zip via `wp plugin install /tmp/buddypress-recaptcha-2.2.0.zip --activate`
...
- [ ] DB version option updates (`wp option get (none - no DB version option; settings live in wp_options under wbc_*)`)
...
- [ ] Cron events re-registered cleanly (`wp cron event list | grep (none - this plugin schedules no cron)`)

## 11 — Release metadata

- [ ] Git tag created: `v2.2.0` (annotated, not lightweight — `git tag -a v2.2.0 -m "..."`)
...
If any post-release signal is red → open a `2.2.0.1` patch cycle immediately.
