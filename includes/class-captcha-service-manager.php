...
class WBC_Captcha_Service_Manager {
	 ...
	 *
	 * @param WBC_Captcha_Service_Interface $service
	 */
	...
	public function verify( $context, $response = null, $args = array() ) {
		...
		if ( null === $response ) {
			...
			$response = isset( $_POST[ $field_name ] ) ? sanitize_text_field( wp_unslash( $_POST[ $field_name ] ) ) : '';
		}
	...
	private function log_debug( $message, $context = '' ) {
		if ( defined( 'WP_DEBUG' ) && WP_DEBUG && defined( 'WP_DEBUG_LOG' ) && WP_DEBUG_LOG ) {
			error_log( sprintf( '[BuddyPress reCAPTCHA Debug] [%s] %s', $context, $message ) );
		}
	}
	...
	private function log_info( $message, $context = '' ) {
		if ( defined( 'WP_DEBUG_LOG' ) && WP_DEBUG_LOG ) {
			error_log( sprintf( '[BuddyPress reCAPTCHA Info] [%s] %s', $context, $message ) );
		}
	}
	...
	private function log_error( $message, $context = '' ) {
		error_log( sprintf( '[BuddyPress reCAPTCHA Error] [%s] %s', $context, $message ) );
	...
	private function show_configuration_notice() {
		if ( current_user_can( 'manage_options' ) ) {
			...
			echo '<p>' . sprintf(
				...
				esc_html__( 'Captcha service is not properly configured. Please %sconfigure your captcha settings%s.', 'buddypress-recaptcha' ),
