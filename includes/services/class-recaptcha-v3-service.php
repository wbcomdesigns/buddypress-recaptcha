...
class WBC_Recaptcha_V3_Service extends WBC_Captcha_Service_Base {
	...
	public function get_site_key() {
		return get_option( 'wc_settings_tab_recapcha_site_key_v3' );
	}
	...
	public function get_secret_key() {
		return get_option( 'wc_settings_tab_recapcha_secret_key_v3' );
	}
	...
	public function verify( $response, $args = array() ) {
		...
		if ( ! empty( $response ) ) {
			...
			$token_field_id = 'wbc_recaptcha_' . $context . '_token';
			$token = isset( $_POST[ $token_field_id ] ) ? sanitize_text_field( $_POST[ $token_field_id ] ) : '';
			...
			if ( empty( $token ) ) {
				...
				foreach ( $legacy_fields as $field ) {
					if ( isset( $_POST[ $field ] ) && ! empty( $_POST[ $field ] ) ) {
						$token = sanitize_text_field( $_POST[ $field ] );
						break;
					}
				}
			}
