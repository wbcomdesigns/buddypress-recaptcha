<?php // phpcs:ignoreFile -- Third-party ALTCHA library file.
...
class AltchaPlugin {
	...
	public function init() {
		...
		if ( defined( 'ALTCHA_VERSION' ) ) {
			self::$version = ALTCHA_VERSION;
		}
		if ( defined( 'ALTCHA_WIDGET_VERSION' ) ) {
			self::$widget_version = ALTCHA_WIDGET_VERSION;
		}
	...
	public function get_api() {
		return trim( get_option( self::$option_api ) );
	}

	public function get_api_custom_url() {
		return trim( get_option( self::$option_api_custom_url ) );
	}
	...
	public function get_blockspam() {
		return get_option( self::$option_blockspam );
	}
	...
	public function get_translations( $language = null ) {
		...
		$translations   = array(
			'error'     => __( 'Verification failed. Try again later.', 'buddypress-recaptcha' ),
			'footer'    => sprintf(
			/* translators: the placeholders contain opening and closing tags for a link (<a> tag) */
				__( 'Protected by %1$sALTCHA%2$s', 'buddypress-recaptcha' ),
				'<a href="' . $ALTCHA_WEBSITE . '" target="_blank">',
				'</a>',
			),
			'label'     => __( 'I\'m not a robot', 'buddypress-recaptcha' ),
			'verified'  => __( 'Verified', 'buddypress-recaptcha' ),
			'verifying' => __( 'Verifying...', 'buddypress-recaptcha' ),
			'waitAlert' => __( 'Verifying... please wait.', 'buddypress-recaptcha' ),
		);
	...
	public function spam_filter_call( $body ) {
		...
		$api_key = $this->get_api_key();
		$resp    = wp_remote_post(
			"https://$api.altcha.org/api/v1/classify",
			array(
